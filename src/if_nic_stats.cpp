...
int get_nic_stats(const char *if_name, nic_stats_t *nic_stats)
{
	int skfd = -1;
	...
	gstrings = (struct ethtool_gstrings *)calloc(1, sz_str + sizeof(struct ethtool_gstrings));

	stats = (struct ethtool_stats*)calloc(1, sz_stats + sizeof(struct ethtool_stats));
	...
	for (unsigned int i = 0; i < n_stats; i++) {
		...
		//printf("get_nic_stats %s: %s = %u\n", if_name, stat_name, (int)stat_val);
		...
		} else if (strncmp(stat_name, "rx_unicast", ETH_GSTRING_LEN) == 0) {
			nic_stats->rx_unicast = stat_val;
